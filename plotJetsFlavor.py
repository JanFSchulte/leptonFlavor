...
def getMuErr(mass, chann, norm=False):
	...
	if norm: 
		...
		znorm = 0.0
		dybkg = 0
	return math.sqrt(0.5**2+lumi**2+znorm**2+pileup**2+dybkg**2+pdf**2+mutrig**2+resolution**2+muid**2)
...
def getElErr(mass, chann, norm=False):
	...
	if norm:
		...
		znorm = 0.0
		dybkg = 0
	return math.sqrt(0.5**2+lumi**2+znorm**2+ pileup**2 + dybkg**2 + pdf**2 + energyscale**2 + idscale**2 + scalefac**2)
...
def plotDataMC(args,plot_mu,plot_el):
	...
	for process in reversed(processes_mu):
		...
		process.theColor = ROOT.kBlue - 3 * processes_mu.index(process)
		process.theLineColor = ROOT.kBlue - 3 * processes_mu.index(process)
		temphist = ROOT.TH1F()
        ...
        for process in reversed(processes_el):
                if not plot_el.muon and "#mu^{+}#mu^{-}" in process.label:
                        process.label = process.label.replace("#mu^{+}#mu^{-}","e^{+}e^{-}")
		process.theColor = ROOT.kRed - 3 * processes_el.index(process)
		process.theLineColor = ROOT.kRed - 3 * processes_el.index(process)
                temphist = ROOT.TH1F()
                temphist.SetFillColor(process.theColor)
                legendHists.append(temphist.Clone)
                legend.AddEntry(temphist,process.label,"f")
	...
	if args.ratio:
		...
		nBinsX = 20
                nBinsY = 10
                hAxis = ROOT.TH2F("hAxis", "", nBinsX, xMin, xMax, nBinsY, 0.0, 10.0)
                hAxis.Draw("AXIS")
	...
	hCanvas.Print("lepFlavor/%s_%s_jets.pdf"%(plot_mu.fileName, year))
